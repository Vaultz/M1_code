...
void* philo_thread_righthanded(void* arg) {
    Philo p = *(Philo*) arg;

    // on bloque l'exécution tant que les 3 threads ne sont pas fournis
    ...
    for(i = 0; i < EAT_TIMES; i++) {
        ...
        printf("phylo %d reçoit la fourchette %d\n", p.num, RIGHT(p.num));
        printf("phylo %d attend la fourchette %d\n", p.num, LEFT(p.num));
...
// ce philosophe prend la fourchette à sa main gauche en priorité
void* philo_thread_lefthanded(void* arg) {
    Philo p = *(Philo*) arg;
...
int main() {
    ...
    pthread_barrier_init(&barriere, NULL, NUM_DINERS);
    ...
    for (i = 0; i < NUM_DINERS; i++) {
        ...
        // on rajoute un gaucher pour empêcher le blocage
        if (i == 0) {
          pthread_create(&philo_threads[i], NULL, philo_thread_lefthanded, &philo_infos[i]);
        }
        else {
          pthread_create(&philo_threads[i], NULL, philo_thread_righthanded, &philo_infos[i]);
